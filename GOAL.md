...
- Re-ran the current primer list with eight flaky runs on unpatched current `main`; only the already-confirmed `dd-trace-py` and `scikit-build-core` cases reproduced there. Among formerly flaky projects, `bokeh`, `cloud-init`, `colour`, `ibis`, `jax`, `meson`, `pandas-stubs`, `PyGithub`, `rotki`, `sockeye`, and `vision` were stable for eight prepared-project runs, and `pydantic` was stable for 50 focused runs. A refreshed full-project `pydantic` probe stack-overflowed in 8/8 runs with only the runtime thread ID changing, so it is a ruled-out exact-panic-output lead under the scope above. `core` reproduced the diagnostic output-order root cause described below rather than a new semantic root cause.
...
- Generated-probe coverage so far: identical parallel two-file checks found no in-scope candidate through seeds 293-2000 after skipping runs where both outputs had essentially the same panic; single-threaded file-entrypoint permutations found no in-scope candidate through 450 two-file seeds and 200 three-file import-chain seeds. Keep widening the probe shape instead of repeating those exact ranges.
...
- A two-run `ecosystem-analyzer` sweep over the current `good.txt` primer list on unpatched `origin/main` found 0 flaky diagnostic locations. It was only a broad sieve: it did not catch the known low-frequency `core` ordering flip in two runs. The only abort leads were `scikit-learn` and `scipy`; prepared-project reruns aborted with the same non-unwinding panic in 8/8 runs for each, so both are ruled-out deterministic panic leads under the scope above.
