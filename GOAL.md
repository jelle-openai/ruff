...
- Re-ran the current primer list with eight flaky runs on unpatched current `main`; only the already-confirmed `dd-trace-py` and `scikit-build-core` cases reproduced there. Among formerly flaky projects, `bokeh`, `cloud-init`, `colour`, `ibis`, `jax`, `meson`, `pandas-stubs`, `PyGithub`, `rotki`, `sockeye`, and `vision` were stable for eight prepared-project runs, and `pydantic` was stable for 50 focused runs. An earlier refreshed full-project `pydantic` probe stack-overflowed in 8/8 runs with only the runtime thread ID changing, so that old output-only variation is ruled out under the scope above. On fresh `upstream/main` at `7c2e9fbc83`, 12 parallel full-project `pydantic` checks exited normally with the same concise hash, so `pydantic` currently gives no panic/no-panic lead. `core` reproduced the diagnostic output-order root cause described below rather than a new semantic root cause.
