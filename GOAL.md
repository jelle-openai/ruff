...
- Generated-probe coverage so far: identical parallel two-file checks found no in-scope candidate through seed 2999 after skipping runs where both outputs had essentially the same panic; single-threaded file-entrypoint permutations found no in-scope candidate through 450 two-file seeds and 200 three-file import-chain seeds. A fresh `upstream/main` six-module full-project probe found no in-scope candidate through seed 275 before being retired, and a lighter six-module cyclic import/class-reference probe found no in-scope candidate or timeout through seed 750. Keep widening the probe shape instead of repeating those exact ranges.
